...
async def render_panel(target, text=None):
    ...
    markup = build_pyrogram_markup(panel_rows())
    if isinstance(target, Message):
        await target.reply_text(body, reply_markup=markup, disable_web_page_preview=True)
        return
    await target.message.edit_text(body, reply_markup=markup, disable_web_page_preview=True)
